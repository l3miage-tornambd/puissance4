...
export type PLAY_FAILURE = Readonly<{success: false, reason: "no such column" | "column is full"}>;

/**
 * Put a token of current player at column colmun.
 * If the play action is impossible, the returned error is chosen with respect to following declaration order.
 ...
 * @returns {success: true, state: GAME_STATE} with the returned GAME_STATE having a changed turn and a new grid.
